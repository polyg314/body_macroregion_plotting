...
#'
#' Creates a new plot of the front and back of the male CHOIR body map.
#'
#' @param df data.frame
#' @param value string
#' @return ggrob
...
#' plot_male_choirbm(cbm_df, "value", male_region_occurences$percent_occurrences, new_color_palette, body_map)
#'
...
plot_male_macroregions_with_counts <- function(df, value, region_occurrences, new_color_palette, body_map) {
  # Exported function to create male body image map colored by macroregion percent and with microregion counts annotated
  #
  # Parameters:
  ...
  # assign male drawing coordinates
...
## use following to print microregion labels on the same image
...
#   print(p2)
  ...
  heading_annotations <- data.frame(
    x = c(142, 147), # Different x values for "Front" and "Back"
    y = c(-10, -10), # Same y value for both, adjust as needed
    label = c("Male Front", "Male Back"),
    group = c("Front", "Back")
  )
